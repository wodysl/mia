			...
			%>
				...
				<h3 class="lb">커뮤니티</h3>
					<ul>
						...
						<li style="background-color: #a2a2a255;"><a href="Humorlist.jsp" style="color: #fff">유머</a></li>
						<li><a href="Userlist.jsp">유저 찾기</a></li>
					</ul>
				</div>
			</div>
		<div class="center">
		<h1 class="title">유머게시판</h1>
...
<%
	...
	try{
		...
		query = "select count(*) from board where category='humor'";
		...
		query = "select board_id,board_img,title,nickname,create_date,view_cnt,good from board where category='humor' order by board_id desc, board_id limit " + start + "," + len;
		PS = con.prepareStatement(query);
		RS = PS.executeQuery();
		...
		<tr onclick="location.href='Content.jsp?id=<%= b_id %>&page=<%= cPage %>'">
			...
			<td><img src="<%= img %>" class="thumbnail1" alt="유머글이미지" /></td>
...
%>
			...
			<div class="footer page_wrap">
				...
				<% }else { %>
					<a href="Humorlist.jsp?page=<%= startPage - 1 %>" tabindex="-1" aria-disabled="true" class="arrow pprev"></a>
				<% 
					}
					for(int i = startPage;i <= endPage; i++){
				%>
					<a href="Humorlist.jsp?page=<%= i %>" class="active"><%= i %></a>
				<% 
					}
				...
				<% }else { %>
					<a href="Humorlist.jsp?page=<%= endPage + 1 %>" class="arrow next"></a>
				<% } %>
				</div>
			</div>
		</div>
