		...
		<div class="left">
			...
			<h3 class="lb">커뮤니티</h3>
				<ul>
					...
					<li style="background-color: #a2a2a255;"><a href="Freelist.jsp" style="color: #fff">자유</a></li>
...
<%
	...
	try{
		...
		query = "select count(*) from board where category='free'";
		...
		query = "select board_id,board_img,title,nickname,create_date,view_cnt,good from board where category='free' order by board_id desc, board_id limit " + start + "," + len;
		PS = con.prepareStatement(query);
		RS = PS.executeQuery();
...
%>
			...
			<div class="footer page_wrap">
				...
				<% }else { %>
					<a href="Freelist.jsp?page=<%= startPage - 1 %>" tabindex="-1" aria-disabled="true" class="arrow pprev"></a>
				<% 
					}
					for(int i = startPage;i <= endPage; i++){
				%>
					<a href="Freelist.jsp?page=<%= i %>" class="active"><%= i %></a>
				<% 
					}
				...
				<% }else { %>
					<a href="Freelist.jsp?page=<%= endPage + 1 %>" class="arrow next"></a>
				<% } %>
				</div>
			</div>
		</div>
