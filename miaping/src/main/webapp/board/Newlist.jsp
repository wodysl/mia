		...
		<div class="left">
			...
			<h3 class="lb">커뮤니티</h3>
				<ul>
					...
					<li style="background-color: #a2a2a255;"><a href="Newlist.jsp" style="color: #fff">최신</a></li>
...
<%
	...
	try{
		...
		query = "select board_id,board_img,title,nickname,create_date,view_cnt,good from board order by board_id desc, board_id limit " + start + "," + len;
		PS = con.prepareStatement(query);
		RS = PS.executeQuery();
...
%>
			...
			<div class="footer page_wrap">
				...
				<% }else { %>
					<a href="Newlist.jsp?page=<%= startPage - 1 %>" tabindex="-1" aria-disabled="true" class="arrow pprev"></a>
				<% 
					}
					for(int i = startPage;i <= endPage; i++){
				%>
					<a href="Newlist.jsp?page=<%= i %>" class="active"><%= i %></a>
				<% 
					}
				...
				<% }else { %>
					<a href="Newlist.jsp?page=<%= endPage + 1 %>" class="arrow next"></a>
				<% } %>
				</div>
			</div>
		</div>
